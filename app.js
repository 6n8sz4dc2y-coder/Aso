...
function normCentreName(v){
  const s=String(v||'').trim().replace(/^RRG Toyota\s+/i,'');
...
function publishImport(){
  ...
  DATA=PENDING_DATA;
  localStorage.setItem('rrgDashboardData', JSON.stringify(DATA));
...
build();
  document.getElementById('adminStatus').innerHTML='<strong>Published.</strong><br>This browser is now using the uploaded data.';
}
function downloadDataBackup(){
  const blob = new Blob([JSON.stringify(DATA,null,2)],{type:'application/json'});
  const a=document.createElement('a');
  a.href=URL.createObjectURL(blob); a.download='rrg-dashboard-data-backup.json'; a.click(); URL.revokeObjectURL(a.href);
}
function resetSavedData(){
  localStorage.removeItem('rrgDashboardData');
  location.reload();
}
...
document.querySelectorAll('nav button').forEach(b=>b.classList.remove('active'));document.querySelectorAll('.section').forEach(s=>s.classList.remove('active'));btn.classList.add('active');document.getElementById(btn.dataset.target).classList.add('active')})});
